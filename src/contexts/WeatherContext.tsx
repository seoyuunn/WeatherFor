...
import { loadWeatherFromStorage, saveWeatherToStorage } from '../services/storage';
...
export const WeatherProvider: React.FC<WeatherProviderProps> = ({ children }) => {
  ...
  const loadWeatherData = async (currentLocation: LocationType, isToday: boolean) => {
    ...
    try {
      ...
      const cachedData = await loadWeatherFromStorage(isToday ? STORAGE_KEYS.LAST_WEATHER : STORAGE_KEYS.LAST_FORECAST);
      
      // If we have cached data less than 1 hour old, use it
      const now = new Date();
      const cacheTime = cachedData?.lastUpdated ? new Date(cachedData.lastUpdated) : null;
      const cacheValid = cacheTime && ((now.getTime() - cacheTime.getTime()) < 3600000); // 1 hour
